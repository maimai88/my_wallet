...
enum _TransferSteps { SelectAccount, EnterAmount, Confirm }
...
class _AccountTransferState extends CleanArchitectureView<AccountTransfer, AccountTransferPresenter> implements AccountTransferDataView {
  ...
  void _onAccountReselect() {
    _controller.animateToPage(0, duration: _pageAnimationDuration, curve: Curves.ease).then((_) {
      presenter.loadAccountDetails(widget.id);
    });
  }
  ...
  void _onAmountUpdate(double amount) {
    this._amount = amount;
    _controller.animateToPage(2, duration: _pageAnimationDuration, curve: Curves.ease).then((_) {
      if (_confirmKey.currentContext != null) _confirmKey.currentState.updateDetail(_toAccount, _fromAccount, _amount);
    });
  }
    ...
    return Column(
      children: <Widget>[
        Padding(
          ...
          child: Text(
            R.string.select_account_to_transfer_to,
        ...
        ListView.builder(
            ...
            itemBuilder: (context, index) => ListTile(
                  ...
                  trailing: Icon(
                    _selected != null && _selected.id == _accounts[index].id ? Icons.radio_button_checked : Icons.radio_button_unchecked,
